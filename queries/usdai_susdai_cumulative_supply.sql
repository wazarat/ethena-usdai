-- sUSDai cumulative daily supply, wrapping Dune query 5563905 (USD.AI: sUSDai Supply).
-- The source query returns raw mint/burn events (block_time, action, amount).
-- To use: fork query 5563905, then replace its body inside the events CTE below.
-- Run `SELECT DISTINCT action FROM ...` first to confirm the action values match.
--
-- Note: the `query_5563905` reference below uses Dune's "query of query" feature.
...
WITH events AS (
  -- <<< paste the SELECT body of query 5563905 here, exposing block_time, action, amount >>>
  SELECT block_time, action, amount FROM query_5563905
),
daily AS (
  SELECT
